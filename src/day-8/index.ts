...
const validSegmentsReverse: Record<string, Digit> = Object.entries(validSegments)
  .reduce((map, entry) => ({
    ...map,
...
class SignalDataDecoder {
  ...
  add(pattern: string) {
    for (const char of pattern) {
      ...
      for (const mapping of inferredMappings) {
        const canExclude = mapping[1].every(c => viableChars.includes(c))
  ...
  getUniqueOutputs() {
    ...
    return this.data.outputs
      .reduce((total, output) => total + (matches.includes(output.length) ? 1 : 0), 0);
  }

  getTotalOutput() {
    const outputNumber = this.data.outputs
      .flatMap(output =>
        validSegmentsReverse[output.split('')
...
async function main() {
  ...
  const [unique, total] = signalData.reduce((sums, data) => {
    const decoder = new SignalDataDecoder(data);
    return [
      sums[0] + decoder.getUniqueOutputs(),
      sums[1] + decoder.getTotalOutput()
    ];
  }, [0, 0]);
